...
describe('where statement', function() {
  ...
  const whereResponse = {from: {user: "user", repo: "repo"}, select: null, take: null};
  ...
  it('should not fail(throw) if where is case insensitive', function() {
    ...
    expect(Parser.parse(input)).toEqual({...whereResponse, where: { filters: [{ field: 'a', filter: 'equals' , value: 'b' }], operators: []}});
	});
  ...
  it('should not fail(throw) if where has many left & right operands and a valid operators', function() {
    const input = "from user/repo where a equals b and c contains d or e not equals f";
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...whereResponse, where: {filters: [{ field: 'a', filter: 'equals' , value: 'b' }, { field: 'c', filter: 'contains' , value: 'd' }, { field: 'e', filter: 'not equals' , value: 'f' }], operators: ["and", "or"]}});
	});
  ...
  it('should not fail(throw) if where has unquoted string as value', function() {
    ...
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...whereResponse, where: {filters: [{ field: 'title', filter: 'equals' , value: 'myTitle' }], operators: []}});
	});
  ...
  it('should not fail(throw) if where has quoted string as value', function() {
    ...
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...whereResponse, where: {filters: [{ field: 'title', filter: 'equals' , value: 'myTitle 1' }], operators: []}});
	});

  it('should not fail(throw) if where has empty string as value', function() {
    const input = 'from user/repo where title equals ""';
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...whereResponse, where: {filters: [{ field: 'title', filter: 'equals' , value: '' }], operators: []}});
	});
  ...
  it('should not fail(throw) if where has unquote string list as value', function() {
    ...
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...whereResponse, where: {filters: [{ field: 'labels', filter: 'equals' , value: ['1', '2', '3'] }], operators: []}});
	});

  it('should not fail(throw) if where has quoted string list as value', function() {
    const input = 'from user/repo where labels equals ["P1", "Priority Medium", "P3"]';
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...whereResponse, where: {filters: [{ field: 'labels', filter: 'equals' , value: ["P1", "Priority Medium", "P3"] }], operators: []}});
	});
