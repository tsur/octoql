...
describe('select statement', function() {
  ...
  const selectResponse = {from: {user: "user", repo: "repo"}, where: null, take: null};
  ...
  it('should not fail(throw) if select has a single fields', function() {
    ...
    expect(Parser.parse(input)).toEqual({...selectResponse, select: ["field"]});
	});
  ...
  it('should not fail(throw) if select has several fields', function() {
    ...
    const result = Parser.parse(input).fields;
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...selectResponse, select: ["field", "field2"]});
	});
  ...
  it('should not fail(throw) if select has several fields', function() {
    ...
    const result = Parser.parse(input).fields;
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...selectResponse, select: ["field", "field2", "field3"]});
	});

  it('should not fail(throw) if select ends with whitespaces', function() {
    const input = "from user/repo select field, field2, field3   , field4,    field5  ";
    const result = Parser.parse(input).fields;
    expect(() => Parser.parse(input)).not.toThrow();
    expect(Parser.parse(input)).toEqual({...selectResponse, select: ["field", "field2", "field3", "field4", "field5"]});
	});
